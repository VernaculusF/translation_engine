...
/// Reuse ImportReport from dictionary_importer.dart
class ImportReport {
  final int total;
...
/// Universal phrase importer supporting CSV, JSON (array) and JSONL
class PhraseImporter {
  final PhraseRepository repository;

  PhraseImporter({required this.repository});
  ...
  Future<ImportReport> importCsv(
    ...
    for (int i = hasHeader ? 1 : 0; i < lines.length; i++) {
      ...
      try {
        ...
        final targetPhrase = _getCol(cols, idx.targetIdx) ?? ((cols.length > 1) ? cols[1] : '');
        final pair = (languagePair ?? _getCol(cols, idx.langIdx)) ?? 'en-ru';
        final category = _getCol(cols, idx.categoryIdx);
        final context = _getCol(cols, idx.contextIdx);
        final confidenceStr = _getCol(cols, idx.confidenceIdx);
        final freqStr = _getCol(cols, idx.freqIdx);
        
        final confidence = int.tryParse(confidenceStr ?? '') ?? 95;
  ...
  Future<ImportReport> importJsonArray(
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      total++;
      try {
        final sourcePhrase = (obj['source_phrase'] ?? obj['source'] ?? '').toString();
        final targetPhrase = (obj['target_phrase'] ?? obj['target'] ?? '').toString();
        final pair = (languagePair ?? obj['language_pair'] ?? '').toString();
        final category = obj['category']?.toString();
        final context = obj['context']?.toString();
        final confidence = (obj['confidence'] is int)
            ? obj['confidence'] as int
            : int.tryParse(obj['confidence']?.toString() ?? '') ?? 95;
  ...
  bool _looksLikeHeader(List<String> header) {
    ...
    return lower.contains('source') ||
        lower.contains('source_phrase') ||
        lower.contains('target') ||
        lower.contains('target_phrase') ||
...
class _ColumnIndex {
  ...
  static _ColumnIndex fromHeader(List<String> header) {
    ...
    return _ColumnIndex(
      sourceIdx: map['source_phrase'] ?? map['source'],
      targetIdx: map['target_phrase'] ?? map['target'],
      langIdx: map['language_pair'] ?? map['lang'] ?? map['pair'],
      categoryIdx: map['category'],
      contextIdx: map['context'],
      confidenceIdx: map['confidence'],
