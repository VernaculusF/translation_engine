...
21) Confidence: Pipeline считает по debugInfo (cacheHits/itemsProcessed), но слои чаще не заполняют поля — метрика псевдо-значима. Фолбек 0.5 вводит в заблуждение.
- Статус: ✖
- Шаги реализации: новая формула (вклад слоёв/покрытие токенов/источник перевода), сбор реальных метрик.
- Тесты/критерии: синтетические кейсы дают ожидаемые значения.

22) hasDataAccess всегда true — маскирует проблемы доступа к данным.
- Статус: ✖
- Шаги реализации: реальная проверка FS/сети; propagate статуса в Engine/слои.
